...
def normalization(x):
    b,c,h,w = x.size()    
    x_flatten = x.flatten()
    x_flatten = x_flatten.view(b,-1)
    
    # max_ = x_flatten * F.sigmoid(x_flatten)
    # min_ = -x_flatten * F.sigmoid(-x_flatten)

    # min_ = x_flatten.min(1)[0]
    # max_ = x_flatten.max(1)[0]
    
    # y = torch.clamp(x, 0, 0.01) * 100
    y = F.relu(x)
    
    
    # min_ = min_.view(b,1,h,w)
    # max_ = max_.view(b,1,h,w)
